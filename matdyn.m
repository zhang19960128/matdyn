...
vol=(8.386494280/2)^3;
for mu=1:atom*3
    %%%%%%%%use the following line when you are using mode displacement%%%%%%%%%%%%%%%%%%%%%%%%%%
    a_mu=renorm(eigvector(atom*(mu-1)+1:atom*(mu-1)+atom,1:end),mass);%%%%%%%%%%%%%%%%
    %%%%%%%%%%%%%%%use this line when you are using eigen vector of the dynamic matrix%%%%%%%%%%
    %a_mu=eigvector(atom*(mu-1)+1:atom*(mu-1)+atom,1:end);
    modeff=zeros(3,1);
    for i=1:atom
